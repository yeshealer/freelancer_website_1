...
class WorldMap extends React.Component {
            ...
            g.selectAll("path")
                ...
                .on("mousemove", function (d) {
                    ...
                    let countryProp = rankingData.filter((x) => {
                        return x.countryId === d.id;
                    });
                    countryProp = countryProp[0];

                    countryProp.countryName = countryProp.countryTextMap.name;

                    if (self.state.scoreDataAllYears === null)
                        return;

                    let countryPropTooltip = self.state.scoreDataAllYears[self.state.selectedYear]['areas'][d.id];
                    ...
                    if (countryProp !== undefined && countryPropTooltip !== undefined) {
                        if (countryProp.weightedScore == -1) {
                            ...
                            let tooltipContainer = "<div class=\"country-flag sprite sprite-" + countryProp['countryId'] + "\"></div>" +
                                ...
                                "<span class='scoreData' style='color:" + selectedColor + "'>" + numeral(countryPropTooltip.dataValue).format("0.0a") + " / " + countryProp.kpiTextMap.denominator + "</span>";
    ...
    setActiveModal(modalId) {
        ...
        let p1 = new Promise(resolve => {
            if (this.state.selectedModal === 'worldRanking') {
                this.setState({
                    selectedGroup: 'GLO',
                    selectedGroupName: this.state.groupListDataMap[this.state.selectedGroup]
                });

                this.mWorldRankingV2.loadWorldRankingData('GLO', this.state.selectedYear, this.state.selectedKPI, resolve);
            } else {
                resolve();
            }
        }).then(() => {
    ...
    onYearChanged(year) {
        ...
        // sdkV2.getKPIValues(this.state.selectedPillar, year, (statusCode, response) => {
    ...
    setGroupStuffByLang() {
        SDK.getGroupList((statusCode, response) => {
            ...
            response.forEach((x) => {
                x.groupName = x.countryGroupTextMap.title;
    ...
    initLoad(initLang, initKpi, initCountry, initYear) {
        const {getKpiScoreMapAction, getRankingKpiAction, kpiObj} = this.props;

        if (initLang === undefined) {
            sdkV2.locale = (this.lang.currentEnglish) ? "en" : "ar";
        } else {
            sdkV2.locale = initLang;
        }

        let initKPIId;
        if (initKpi === undefined) {
            initKPIId = getCookie("kpiId");
        } else {
            initKPIId = initKpi;
        }
        ...
        .then(value => {
            ...
            // };
            //
            // SDK.getCountryAlphaListAction((statusCode, response) => {
        ...
        .then(value => {
            if (initKPIId !== null) {
                getKpiScoreMapAction(initKPIId)
                .then((response) => {
                ...
                getRankingKpiAction(this.state.selectedKPI, this.state.selectedYear, this.state.selectedGroup)
                .then((response) => {
                    this.setState({
                        rankingData: response.rankingKpiList,
                        selectedGroupRankingData: response.rankingKpiList
                    });
                });
            }

            sdkV2.getCountryList()
            .then((response) => {
                let countryIdMap = {};
                response.data.forEach((x) => {
                    countryIdMap[x.countryId] = x;
                    x.countryName = x.countryTextMap.name;
                });

                this.setState({countries: response.data, countryIdMap: countryIdMap});
            }).catch(err => console.log(err));
    ...
    componentDidMount() {
        ...
        const { getRealmListAction, getWholeRealmTreeAction, getRankingKpiAction } = this.props;


        getRealmListAction().then(() => {
        }).catch((err) => {
        });

        getWholeRealmTreeAction().then(() => {
        }).catch((err) => {
        });
    ...
    render() {
        return (
            <div className="worldMapWrapperM">
                ...
                {(() => {
                    if (!this.state.isDesktopMode) {
                        ...
                        return [
                            ...
                            // <div key="badge_div" className="betaBadge"/>
                        ];
                    }
                })()}
                ...
                {this.renderKpiSideMenu()}

                {this.renderWorldRankingSideMenu()}

                {this.renderCompareSideMenu()}

                {this.renderGenerateGraphs()}
...
export default connect(mapStateToProps, actions)(WorldMap);
